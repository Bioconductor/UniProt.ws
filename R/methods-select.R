    ...
    function(list, joinType="left")
{
    for(i in seq_len(length(list))) {
        if (i==1) {
            ...
            if(joinType=="left") {
                fin <-
                    merge(fin, list[[i]], by="ACC+ID", all.x=TRUE) ## left outer join
            }
            else if(joinType=="all") {
                fin <-
                    merge(fin, list[[i]], by="ACC+ID", all=TRUE) ## full outer join
            }
        }
    }
    fin
